...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    window.continue_on_mouse_click()
    x=abs(rectangle.corner_1.x-rectangle.corner_2.x)/2
    y=abs(rectangle.corner_1.y-rectangle.corner_2.y)/2
    line=rg.Line(rg.Point(rectangle.get_center().x-x,rectangle.get_center().y+y),rg.Point(rectangle.get_center().x+x,rectangle.get_center().y-y))
    line.arrow='first'
...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n):
        x = abs(rect.corner_1.x - rect.corner_2.x)/2
        y = abs(rect.corner_1.y - rect.corner_2.y)/2
        rectangle=rg.Rectangle(rg.Point(rect.get_center().x-(delta*k)-x,rect.get_center().y-(delta*k)-y),rg.Point(rect.get_center().x+(delta*k)+x,rect.get_center().y+(delta*k)+y))
